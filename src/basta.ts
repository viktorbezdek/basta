...
export function basta(options: IOptions) {
  ...
  const testPath = i => /.*\/(test|mock|types)s?\.ts/gi.test(i)

  stream.on("data", ({ path }) => {
    if (lstatSync(path).isFile() && !testPath(path)) {
  ...
  stream.on("end", () => {
    ...
    if (!options.debug) {
      ...
      if (options.threshold && options.threshold <= rate) {
        console.error(
          `Error: duplication rate(${rate}%) less then threshold(${options.threshold}%)`
            .bold.red
        )
        process.exit(1)
      }
